...
const CACHE_NAME = CACHE_PREFIX + 'v6-' + new URL(self.registration.scope).pathname;
const assetUrl = path => new URL(path, self.registration.scope).href;
const CORE_ASSETS = ['', 'index.html', 'css/style.css', 'js/ui.js', 'js/adminRecovery.js', 'js/editorGuard.js', 'js/invitations.js', 'js/audioSynth.js', 'js/confetti.js', 'js/odometer.js', 'js/app.js', 'js/admin.js', 'manifest.json'].map(assetUrl);
...
self.addEventListener('activate', event => event.waitUntil(caches.keys().then(keys => Promise.all(keys.filter(key => key.startsWith(CACHE_PREFIX) && key !== CACHE_NAME && (!key.includes('v6-') || key.endsWith(new URL(self.registration.scope).pathname))).map(key => caches.delete(key)))).then(() => self.clients.claim())));
