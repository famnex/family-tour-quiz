...
const CACHE_NAME = CACHE_PREFIX + 'v13-' + new URL(self.registration.scope).pathname;
...
self.addEventListener('activate', event => event.waitUntil(caches.keys().then(keys => Promise.all(keys.filter(key => key.startsWith(CACHE_PREFIX) && key !== CACHE_NAME && (!key.includes('v13-') || key.endsWith(new URL(self.registration.scope).pathname))).map(key => caches.delete(key)))).then(() => self.clients.claim())));
